...
SPECIAL_PREFIX_DEFAULT="s3://bidgely-artifacts2/Murali_Users/special/teco"
...
SPECIAL_PREFIX="${ITRON_SPECIAL_REQUEST_S3_PREFIX_TECO:-$SPECIAL_PREFIX_DEFAULT}"
...
if [ -n "$SPECIAL_FILE" ]; then
  ...
  echo "Special request detected for TECO: ${METER_LIST_S3}"
  echo "Running TECO special request first, then normal daily flow."
  ./support_box/run_special_request_and_create_pr.sh \
    --pilot teco \
    --request-name "$REQUEST_NAME" \
    --meter-list-s3 "$METER_LIST_S3" \
    --date "$DATE_STR"
  ./support_box/run_teco_daily.sh
else
  echo "No TECO special request found for ${DATE_STR}. Running normal daily flow."
  ./support_box/run_teco_daily.sh
fi
